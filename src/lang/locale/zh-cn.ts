export default {
	LicensePurchaseInfo:
		"您需要购买许可证才能使用IOTO的所有功能。请联系作者获取更多信息。",
	AuthorWechatID: "作者微信ID: johnnylearns",
	"You must provide an API Key to run this command":
		"运行此命令必须提供 API 密钥。",
	"You need to provide the email for your account to run this command":
		"运行此命令需要提供账户关联的电子邮件。 ",
	"Update Core Files": "更新核心文件",
	"Update Help Docs": "更新帮助文档",
	"Update MYIOTO Templates": "更新MYIOTO模板",
	"Update CSS Snippets": "更新CSS代码片段",
	"Update IOTO Framwork Setting Plugin": "更新IOTO框架设置插件",
	"Your Update API Key": "你的 API 密钥",
	"Please enter your update API Key": "请输入你的更新 API 密钥",
	"Enter your update API Key": "输入你的更新 API 密钥",
	"Updating, plese wait for a moment": "正在更新，请稍等。",
	"Your API Key was expired. Please get a new one.":
		"你的API密钥已过期，请获取新密钥。",
	"Getting Data ……": "正在获取数据……",
	"There are": "还有",
	Got: "已获取",
	records: "条记录",
	"files needed to be updated or created.": "需要更新或创建的文件。",
	"Failed to write file: ": "写入文件失败：",
	"files needed to be processed.": "需要处理的文件。",
	"All Finished.": "全部完成。",
	"Your Email Address": "你的电子邮件地址",
	"Please enter the email you provided when you purchase this product":
		"请输入您购买此产品时提供的电子邮件地址",
	"Enter your email": "输入你的电子邮件",
	"IOTO Framework Path": "IOTO框架路径",
	"Please enter the path to your IOTO Framework": "请输入你的 IOTO 框架路径",
	"Enter the path to your IOTO Framework": "输入你的IOTO框架路径",
	"IOTO Update Settings": "IOTO更新设置",
	Basic: "基本设置",
	UserSyncTemplates: "用户同步模板",
	"User Sync Configration Update Settings": "用户同步配置更新设置",
	"Your Airtable Personal Token": "你的Airtable个人令牌",
	"Please enter your Airtable Personal Token": "请输入你的 Airtable 个人令牌",
	"Enter your Airtable Personal Token": "输入你的Airtable个人令牌",
	"Your Sync Setting URL": "你的同步设置网址",
	"Please enter the url of your sync setting table":
		"请输入你的同步设置表的网址",
	"Enter the url": "输入网址",
	"Your Sync Templates Folder": "你的同步模板文件夹",
	"Please enter the path to your sync templates folder":
		"请输入同步模板文件夹的路径",
	"Enter the path to your sync templates folder":
		"输入你的同步模板文件夹的路径",
	"Get Your Personal Sync Templates": "获取您的个人同步模板",
	"You need to provide the airtable url for your sync setting table to run this command":
		"你需要提供你的同步设置表的网址来运行此命令",
	"When you use the sync with online database feature of IOTO, the sync configration generater I built could help you a lot.":
		"当你使用IOTO中提供的与在线数据库同步的功能时，我开发的同步配置生成器，可以很好地帮到你。",
	"You can use the following link to open the shared base and save it to your own Airtable workspace.":
		"你可以通过下面的链接打开这个生成器的分享链接，并把它保存到你自己的Airtable工作区中。",
	"Sync Configration Generator": "同步配置生成器",
	"You can watch the follow video to find out how to use this sync configration base.":
		"你可以通过观看下面的视频去学习如何使用这个同步配置生成器。",
	"How to use the sync configration generator": "同步配置生成器用法演示",
	IOTOFrameworPath: "0-辅助/IOTO",
	UserSyncTemplatesPath: "0-辅助/IOTO/Templates/Templater/MyIOTO/同步模板",
	Executing: "正在执行",
	completed: "完成",
	failed: "失败",
	"Deploy IOTO With One Click": "一键部署IOTO ⭐",
	"Get IOTO Framework Setting Plugin": "获取IOTO框架设置插件",
	"Get CSS Snippets": "获取CSS代码片段",
	"Get Core Files": "获取核心文件",
	"Get MYIOTO Templates": "获取MYIOTO模板",
	"Get Help Docs": "获取帮助文档",
	"Help documents updated today": "今天更新的帮助文档",
	"Help documents updated in the past week": "过去一周更新的帮助文档",
	"Help documents updated in the past two weeks": "过去两周更新的帮助文档",
	"Help documents updated in the past month": "过去一个月更新的帮助文档",
	"All help documents": "全部帮助文档",
	"In order to help you to learn how to use IOTO especially the sync with online database feature, I will keep posting instructions and videos to the following link.":
		"为了帮助你学习如何使用IOTO，特别是与在线数据库同步的功能，我将不断发布相关教程和视频到以下链接。",
	"IOTO How To Guide": "IOTO使用教程",
	"Validating...": "正在验证...",
	"Valid API Key": "有效API密钥",
	"Valid Email": "有效电子邮件",
	"IOTO Running Language": "IOTO框架运行语言",
	"Please Chose Your IOTO Framework Running Language":
		"请选择你的IOTO框架运行语言",
	"Auto (Follow System Language)": "根据Obsidian的语言自动选择",
	"Reload OB": "重新加载",
	"Update User Permissions": "更新用户权限",
	"Update User Permissions Success": "更新用户权限成功",
	"Update User Permissions Failed": "更新用户权限失败",
	"Updating User Permissions ...": "正在更新用户权限 ...",
	IOTO_UPDATES: "IOTO更新记录",
	IOTO_TOTURIALS: "IOTO使用教程",
	"Install IOTO Template Generator": "安装IOTO模板生成器",
	"Install Sync Scripts Generator": "安装同步脚本生成器",
	"Invalid GitHub repository URL": "无效的 GitHub 仓库链接",
	"Invalid Gitee repository URL": "无效的 Gitee 仓库链接",
	"Checking for updates from": "正在检查更新：",
	"No release found for this repository": "未找到此仓库的发布版本",
	"Release is missing manifest.json or main.js. Cannot install.":
		"发布版本缺少 manifest.json 或 main.js，无法安装。",
	"Invalid manifest.json: missing 'id' field":
		"无效的 manifest.json：缺少 'id' 字段",
	Plugin: "插件",
	"installed/updated successfully": "安装/更新成功",
	reloaded: "已重新加载",
	"Automatic reload failed": "自动重载失败",
	"Plugin updated but reload failed":
		"插件已更新，但自动重载失败，请手动重启 Obsidian 或重新启用插件。",
	"Plugin installed, please enable manually":
		"插件安装完成，请在设置中手动启用。",
	"Failed to install plugin": "安装插件失败",
	"Check console for details": "请查看控制台了解详情",
	"is already up to date": "已经是最新版",
	"Current Version": "当前版本",
	"Check for Updates": "检查更新",
	"Checking...": "正在检查...",
	"Already up to date": "已经是最新版本",
	"Update available": "有可用更新",
	"Start Update": "开始更新",
	"You are using a development version": "您正在使用开发版本",
	"Failed to check for updates": "检查更新失败",
	"Updating...": "正在更新...",
	Updated: "更新完成",
	"Restart Obsidian to apply changes": "重启 Obsidian 以应用更改",
	"Reloading Obsidian to apply update...":
		"正在重新加载 Obsidian 以应用更新...",
	"Install IOTO Dashboard": "安装IOTO仪表板",
	"Install IOTO Tasks Center": "安装IOTO任务中心",
	"Install Text Popup": "安装Text Popup",
	"Install My Text Tools": "安装My Text Tools",
	"Install SlidesRup": "安装SlidesRup",
	"Install Airtable Fetcher": "安装Airtable Fetcher",
	"Install Feishu Fetcher": "安装Feishu Fetcher",
	"Install Vika Fetcher": "安装Vika Fetcher",
	"Downloading manifest": "正在下载清单文件...",
	"Downloading plugin files": "正在下载插件文件...",
	PluginIndicator: " (插件)",
	API_Token_Invalid: "API密钥无效",
	"Plugin Download Source": "插件下载源",
	"Choose where to download and update plugins": "选择插件下载和更新的源",
	GitHub: "GitHub",
	Gitee: "Gitee",
	"Update Skills": "更新技能",
	TRAE: "TRAE",
	Workbuddy: "Workbuddy",
	DSH: "DeepSeek Harness",
	Qoder: "Qoder",
	PI: "Pi",
	"Oh My Pi": "Oh My Pi",
	Codebuddy: "Codebuddy",
	"Claude Code": "Claude Code",
	OpenCode: "OpenCode",
	"OpenAI Codex": "OpenAI Codex",
	"Mimo Code": "Mimo Code",
	Hermes: "Hermes",
	"Select AI Agent Platform": "选择 AI Agent 平台",
	"No platform selected": "未选择平台",
	"Plugins Center": "插件中心",
	"IOTO Plugins Center Description": "浏览、安装和更新 IOTO 辅助插件。",
	"Not installed": "未安装",
	"Installed": "已安装",
	Install: "安装",
	Update: "更新",
	"Latest version": "最新版本",
	"Installed version": "已安装版本",
	"Failed to check plugin info": "获取插件信息失败",
	Retry: "重试",
	"Installing...": "正在安装...",
	"Check for updates": "检查更新",
	"IOTO Template Generator": "IOTO 模板生成器",
	"IOTO Template Generator.desc": "可视化生成 IOTO 模板并导出到你的库中。",
	"Sync Scripts Generator": "同步脚本生成器",
	"Sync Scripts Generator.desc": "为 IOTO 的在线数据库同步功能生成同步配置脚本。",
	"IOTO Dashboard": "IOTO 仪表板",
	"IOTO Dashboard.desc": "用于概览你 IOTO 框架使用情况与统计数据的仪表板。",
	"IOTO Tasks Center": "IOTO 任务中心",
	"IOTO Tasks Center.desc": "在 IOTO 框架内管理和追踪你的任务。",
	"Text Popup": "Text Popup",
	"Text Popup.desc": "将选中的文字以浮动窗口弹出，便于专注阅读。",
	"My Text Tools": "My Text Tools",
	"My Text Tools.desc": "一组用于文本处理的 Obsidian 实用工具。",
	"SlidesRup": "SlidesRup",
	"SlidesRup.desc": "在 Obsidian 中直接创建并演示幻灯片。",
	"Airtable Fetcher": "Airtable Fetcher",
	"Airtable Fetcher.desc": "抓取 Airtable 记录并嵌入到你的笔记中。",
	"Feishu Fetcher": "Feishu Fetcher",
	"Feishu Fetcher.desc": "抓取飞书 / Lark 文档与数据并嵌入到你的笔记中。",
	"Vika Fetcher": "Vika Fetcher",
	"Vika Fetcher.desc": "抓取维格表记录并嵌入到你的笔记中。",
};
